...
{
	public partial class KaazingWsXamarinDemo_iOS_UnifiedViewController : UIViewController
	{
		...
		public KaazingWsXamarinDemo_iOS_UnifiedViewController (IntPtr handle) : base(handle)
		{
		}
			...
			connectButton.TouchUpInside += async (sender, e) => {
				try {
					if (!_isConnected) {						
						...
						Log("CLOSING");// InvokeOnMainThread is inside
						_webSocket.Close();
					}
		...
		{
			private KaazingWsXamarinDemo_iOS_UnifiedViewController _controller;
			...
			public LoginHandlerDemo(KaazingWsXamarinDemo_iOS_UnifiedViewController controller)
			{
